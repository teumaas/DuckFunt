...
namespace SpaceHuntClasses
{
    class Utility
    {
        ...
        public static string GetStringBordersFormat(SpriteFont font, string text, int width)
        {
            var result = "";
            int add = 0;
            ...
            for(int i = 0; i < text.Count(); i++)
            {
                ...
                if(font.MeasureString(result).X - add>= width - font.MeasureString(" ").X)//if collision with border
                {
                    result += "\n";
                    add = (int)font.MeasureString(result).X;
